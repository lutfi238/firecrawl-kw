...
export function useToolExecutorWithActivity() {
  ...
  const addStep = (id: string, label: string): ActivityStep => {
    const step: ActivityStep = { id, label, status: "active", startedAt: Date.now() };
  ...
  const execute = useCallback(async (toolName: string, args: Record<string, unknown>) => {
    ...
    try {
      ...
      updateStep("prepare", { status: "done", detail: "Ready", completedAt: Date.now() });
      ...
      updateStep("call", {
        ...
        detail: res.isError ? "Failed" : `Done (${(duration / 1000).toFixed(1)}s)`,
      ...
      try {
        ...
        updateStep("log", { status: "done", detail: "Logged", completedAt: Date.now() });
      } catch {
        updateStep("log", { status: "error", detail: "Log failed", completedAt: Date.now() });
      }
      ...
      setError(message);
      updateStep("call", { status: "error", detail: message, completedAt: Date.now() });
    } finally {
      clearTimeout(timeout);
      setLoading(false);
  ...
  const cancel = useCallback(() => {
    ...
    setSteps((prev) => prev.map((s) =>
      s.status === "active" ? { ...s, status: "error", detail: "Cancelled", completedAt: Date.now() } : s
    ));
  }, []);
