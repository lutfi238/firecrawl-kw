...
export async function handleRegister(
  ...
  try {
    body = (await req.json()) as ClientRegistrationRequest;
  } catch {
    return json({ error: "invalid_client_metadata" }, corsHeaders, 400);
  }
...
export async function handleAuthorizePost(
  ...
  const code_challenge_method = String(
    form.get("code_challenge_method") || "plain",
  );
  ...
  if (!expected || password !== expected) {
    ...
    return new Response(
      htmlPage(
        "Denied",
        `<p>Wrong password.</p><p><a href="javascript:history.back()">Try again</a></p>`,
      ),
      {
        status: 401,
        headers: { ...corsHeaders, "Content-Type": "text/html" },
...
export async function handleToken(
  ...
  if (!client) {
    console.warn(
      "[oauth] token invalid_client client_id=",
      client_id || "missing",
    );
  ...
  if (client.client_secret_hash) {
    if (!client_secret) {
      console.warn("[oauth] token missing_client_secret client_id=", client_id);
      return json({ error: "invalid_client" }, corsHeaders, 401);
    }
    const hash = await sha256(client_secret);
    if (hash !== client.client_secret_hash) {
      console.warn("[oauth] token invalid_client_secret client_id=", client_id);
      return json({ error: "invalid_client" }, corsHeaders, 401);
    }
  }
  ...
  if (grant_type === "authorization_code") {
    ...
    if (resource && resource !== getBaseUrl(req)) {
      console.warn("[oauth] invalid_target resource=", resource);
      return json({ error: "invalid_target" }, corsHeaders, 400);
    }
...
async function findOAuthClient(
  ...
  const envClientId =
    Deno.env.get("CLAUDE_OAUTH_CLIENT_ID") ||
    Deno.env.get("MCP_OAUTH_CLIENT_ID") ||
    "";
  if (envClientId && clientId === envClientId) {
    const envSecret =
      Deno.env.get("CLAUDE_OAUTH_CLIENT_SECRET") ||
      Deno.env.get("MCP_OAUTH_CLIENT_SECRET") ||
      "";
    const rawRedirects =
      Deno.env.get("CLAUDE_OAUTH_REDIRECT_URIS") ||
      Deno.env.get("MCP_OAUTH_REDIRECT_URIS") ||
      DEFAULT_CLAUDE_REDIRECT_URI;
    ...
    return {
      ...
      client_secret_hash: envSecret ? await sha256(envSecret) : null,
      name: Deno.env.get("CLAUDE_OAUTH_CLIENT_NAME") || "Claude Web",
      redirect_uris:
