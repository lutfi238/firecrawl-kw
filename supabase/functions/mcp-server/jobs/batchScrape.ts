...
export async function processBatchScrapeJob(jobId: string, args: Record<string, unknown>) {
  const svc = getServiceClient();
  try {
    await svc.from("mcp_jobs").update({ status: "processing" }).eq("id", jobId);

    const urlList = ((args.urls as string) || "").split(",").map((url: string) => url.trim()).filter(Boolean);
    ...
    for (const url of urlList) {
      try {
        ...
        results.push({ url, title: "Error", markdown: "", error: error instanceof Error ? error.message : "unknown" });
      }
    ...
    await svc.from("mcp_jobs").update({
      status: "completed",
      output: { results, totalScraped: results.filter((result) => !result.error).length },
    }).eq("id", jobId);
  } catch (error) {
    await svc.from("mcp_jobs").update({
      status: "failed",
